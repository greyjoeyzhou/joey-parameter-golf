...
Shared instructions for coding agents working in this repo, including Claude Code and OpenCode.
...
After every training run completes, commit automatically. If your runtime requires explicit commit approval, treat post-run archival commits in this repo as pre-approved and use this format:
...
- `.claude/settings.json` contains Claude-specific permission rules only; it is not the source of truth for repo instructions.
