...
_OriginalTrainAndEval = module.train_and_eval
...
def _skip_quantized_eval() -> bool:
    return _flag_enabled("SKIP_QUANTIZED_EVAL") or _flag_enabled("COMPETITION_MODE")
...
def _reset_rotary_caches(base_model):
    for block in getattr(base_model, "blocks", []):
        rotary = getattr(getattr(block, "attn", None), "rotary", None)
        if rotary is None:
            continue
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    token_count += float(wlen - s)
                    if getattr(val_data, "val_byte_counts", None) is None:
                        byte_count += _sum_lut_bytes(val_data, y_batch[i, s:wlen], x_batch[i, s:wlen], device)
                    else:
                        byte_slice = val_data.val_byte_counts[ws + s:ws + wlen].to(
                            device=device, dtype=torch.float64, non_blocking=True
                        )
...
def train_and_eval_with_sidecar(h, device):
    if not _skip_quantized_eval():
        return _OriginalTrainAndEval(h, device)

    module.random.seed(h.seed)
    module.np.random.seed(h.seed)
    torch.manual_seed(h.seed)
    torch.cuda.manual_seed_all(h.seed)

    val_data = ValidationDataWithSidecar(h, device)
    module.log(f"train_shards: {len(list(Path(h.datasets_dir).resolve().glob('fineweb_train_*.bin')))}")
    module.log(f"val_tokens: {val_data.val_tokens.numel() - 1}")

    base_model, compiled_model = module.train_model(h, device, val_data)
    torch._dynamo.reset()
    module.timed_eval("pre-quantization post-ema", module.eval_val, h, device, val_data, compiled_model)

    module.serialize(h, base_model, Path(__file__).read_text(encoding="utf-8"))
    if h.distributed:
        dist.barrier()

    # Competition-mode runs only need the serialized artifact; recompiling the
    # quantized model for a final eval adds minutes of tail latency on 8xH100.
    module.log("quantized_eval: skipped by competition mode")
...
module.train_and_eval = train_and_eval_with_sidecar
